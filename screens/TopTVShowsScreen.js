...
export default function TopTVShowsScreen({ navigation }) {
  ...
  async function fetchShows(p) {
    try {
      const res = await axios.get(
        `https://api.themoviedb.org/3/discover/tv?api_key=${API_KEY}&sort_by=vote_average.desc&vote_count.gte=100&page=${p}`
      );
  ...
  function loadMore() {
    const next = page + 1; setPage(next); setLoadingMore(true); fetchShows(next);
  }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>🏆 Top TV Series</Text>
      </View>
      <FlatList
        data={shows}
        numColumns={3}
