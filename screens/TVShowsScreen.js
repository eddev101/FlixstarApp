...
export default function TVShowsScreen({ navigation }) {
  ...
  useEffect(() => { fetchGenres(); fetchShows(1, activeFilters); }, []);
  ...
  async function fetchShows(p, f) {
    try {
      let url = `https://api.themoviedb.org/3/discover/tv?api_key=${API_KEY}&language=en-US&page=${p}`;
      if (f.genre) url += `&with_genres=${f.genre}`;
      if (f.year) url += `&first_air_date_year=${f.year}`;
  ...
  function applyFilters() {
    setShowFilters(false); setLoading(true); setPage(1);
    setActiveFilters({ ...filters }); fetchShows(1, filters);
  }

  function resetFilters() {
    const def = { genre: '', year: '', sort: 'popularity.desc', provider: '', country: '', rating: '0' };
    setFilters(def); setActiveFilters(def); setShowFilters(false); setLoading(true); setPage(1);
    fetchShows(1, def);
  }

  function loadMore() {
    const next = page + 1; setPage(next); setLoadingMore(true); fetchShows(next, activeFilters);
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity style={styles.card} onPress={() => navigation.navigate('TVShowDetail', { id: item.id })}>
            ...
            <Text style={styles.cardYear}>{item.first_air_date?.slice(0, 4)}</Text>
          </TouchableOpacity>
        )}
      />
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Filter TV Shows</Text>
