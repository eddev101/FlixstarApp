...
export default function CollectionDetailScreen({ route, navigation }) {
  ...
  if (!collection) {
    return (
      <View style={styles.loader}>
        <Text style={{ color: '#fff' }}>Failed to load collection</Text>
      </View>
    );
  }
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.content}>
        ...
        <View style={styles.topRow}>
          <Image
            source={{
              uri: collection.poster_path
                ? `https://image.tmdb.org/t/p/w342${collection.poster_path}`
                : 'https://via.placeholder.com/110x165'
            }}
            style={styles.poster}
          />
          <View style={styles.info}>
            <Text style={styles.title}>{collection.name || 'Unknown'}</Text>
            <Text style={styles.moviesCount}>{parts.length} Movies</Text>
          </View>
        </View>
